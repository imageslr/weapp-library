...
const TEMPLATE_DIR = __dirname + "/data"
...
filelist.forEach(absolutePath => {
  console.log(`[INFO] Reading mock template file: ${absolutePath}`)
  try {
    ...
    console.log(`failed to read mock template file: ${absolutePath}`, err)
  }
})
...
routes.forEach(item => {
  ...
  const handler = (req, res) => {
    ...
    if (data._res) { // 自定义响应 Code
      let _res = data._res
      if (_res.status && parseInt(_res.status, 10) !== 200 && _res.data) data = _res.data
  ...
  console.log(`[INFO] Add mock api, method: ${method} url: ${pathname}`)
})
